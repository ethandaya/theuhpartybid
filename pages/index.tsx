...
import {FullComponents, MediaConfiguration, NFTDataContext, NFTFullPage, NFTPreview} from '@zoralabs/nft-components'
...
const CheckOwnerComponent = ({wallet}: {wallet: string}) => {
  ...
  return <span style={{color: 'red', fontWeight: 'bold'}}>You need to own the token before auctioning it off :(</span>;
}
...
export default function Home() {
  ...
  const [network, setNetwork] = useState({chainId: null, networkName: null});
  ...
  useEffect(() => {
    if (network.networkName !== null) {
      ...
      setAddress({
        ...
        auctionHouse: auctionHouse[network.networkName as any]
      });
    }
  }, [active, library, setAddress, network])


  const getApproved = useCallback(async (zid: string) => {
    if (!network.chainId || !zid || !library) {
      return;
    }
    console.log('getting approved');

    try {
      const zdk = new Zora(library, network.chainId as any);
      const owner = await zdk.fetchOwnerOf(zid);
      console.log({owner})
      const result = await zdk.fetchIsApprovedForAll(owner, address.auctionHouse);
  ...
  const setApprove = useCallback(async () => {
    ...
    console.log('setting approved', zid, address.auctionHouse);
  ...
  const updateZoraToken = useCallback((evt: any) => {
    if (evt.target.value) {
  ...
  const setupAuction = useCallback(async (e) => {
      e.preventDefault()
      try {
        setError(undefined)
        if (!active) {
        ...
        const auctionHouse = new AuctionHouse(library.getSigner(), network.chainId as any)
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="So you wanna do some fancy auctions"/>
      ...
      <main className={styles.main}>
        <div style={{padding: 20}}>
          ...
          <h3>
            step 1:
            paste a link to the zora token you want to auction off
            <input type="text" onChange={updateZoraToken} />
          </h3>
          {zid && library && (
            <MediaConfiguration networkId={(network.chainId || 1).toString() as any}>
            <NFTFullPage
            ...
            </NFTFullPage></MediaConfiguration>
          )}
        </div>
        {
          library && zid && (
          <div className={`${styles.approveContainer}`}>
          <h1 style={{fontWeight: 'bold'}}>step 2: approve token for auction house</h1>
          <div>
              Token is {isApproved ? 'approved' : 'not approved'} for auction house sales.
              <br />
              {!isApproved && (
        ...
        <div className={`${styles.bidContainer}`}>
          ...
          <form style={{margin: '40px 0'}} onSubmit={setupAuction}>
            <label
              style={{fontWeight: 'bold', textTransform: 'uppercase', fontSize: 14}}>How many days</label>
            <input type="number" step={1} max={90} value={days} onChange={(e: any) => setDays(e.target.value)}/>
            <br />

            What's the ETH reserve price:
            <input type="number" value={amount} step={0.01} onChange={(e: any) => setAmount(e.target.value)}/>
            <br />
            ...
            <input type="text" disabled={!curator} step={1} max={100} min={0} value={curatorFee} onChange={(e) => setCuratorFee(parseInt(e.target.value))}/>
            <br /></div>
            )}
          ...
          <div style={{ marginTop: 'auto'}}>
            proof of concept.<br />
            please verify txns and report bugs on github<br />
            <br />
            <a rel="noreferrer" href="https://github.com/iainnash/zuctioneer" target="_blank">github.com/iainnash/zuctioneer</a>
          </div>
        </div>
          )}
      </main>
